    ...
    for i in range(num_holdings):
        ...
        with cols[0]:
            ticker = st.selectbox(
                ...
                key=f"ticker_{i}",
                index=available_tickers.index("AAPL") if i == 0 else
                      available_tickers.index("XOM") if i == 1 else
                      available_tickers.index("LMT") if i == 2 else 0
            )
        with cols[1]:
            weight = st.number_input(
                ...
                min_value=0.0,
                max_value=100.0,
                value=30.0 if i == 0 else 50.0 if i == 1 else 20.0,
                key=f"weight_{i}"
            )
...
if st.button("Calculate Risk", type="primary"):
    if abs(total_weight - 100) > 1:
        ...
        most_exposed = portfolio_with_stats.loc[
            (portfolio_with_stats["worst_drawdown"].abs() * portfolio_with_stats["weight"] / 100).idxmax(),
            "ticker"
        ]
...
# ── Section 4: Sector Reaction Heatmap ───────────────────────────
