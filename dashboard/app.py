...
def fetch_latest_gdelt_news():
    try:
        ...
        def get_category(themes):
            t = str(themes)
            if any(k in t for k in ["MILITARY", "WAR", "WEAPON", "MISSILE", "INVASION", "AIRSTRIKE"]):
                return "Military"
...
with c1:
    ...
    <div class='card'>
        ...
        <div class='{delta_class}'>{delta_sign}{delta_score:.1f} since yesterday</div>
    </div>
    """, unsafe_allow_html=True)
...
with col_chart:
    ...
    st.markdown("<div class='card'><div class='chart-title'>Geo-Tension Index — last 30 days</div>", unsafe_allow_html=True)
...
with col_news:
    ...
    if len(news_df) > 0:
        for _, row in news_df.head(5).iterrows():
            ...
            source = str(row["source"])[:45] if pd.notna(row["source"]) else "Unknown source"
            news_html += f"""
            <div class='news-item'>
